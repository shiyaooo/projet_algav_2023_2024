...
typedef struct tournoisB{
    ...
    int degre;  // Degré du nœud (hauteur de l'arbre)
    int pfd;
    struct tournoisB* parent; // Pointeur vers le parent
    struct tournoisB* enfant; // Pointeur vers le premier enfant
    struct tournoisB* frere; // Pointeur vers le prochain frère de ce noeud
} TournoisB;
...
void updateDepth(TournoisB *node, int newDepth);
TournoisB *mergeFB_meme_degre(TournoisB *tree1, TournoisB *tree2);
//TournoisB *insert_cle_in_TB(TournoisB *tb, Cle_entier* cle) ;
